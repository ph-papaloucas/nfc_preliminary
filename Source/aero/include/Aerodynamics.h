...
public:
  ...
  template <typename T>
  T getAoa1(std::array<T, 2> velocity) { // this is without theta
    T aoa = -atan2(velocity[1], velocity[0]);
    return aoa;
  }

  template <typename T>
  static T ground_effect_cl_increase_factor(T height, double wingspan,
                                            double AR) {
    return 1 + (288 * pow(height / wingspan, 0.787) *
                exp(-9.14 * pow(height / wingspan, 0.327)) / pow(AR, 0.882));
  }
  ...
  std::array<double, 2> getAeroForcesEarthframe(std::array<double, 2> velocity,
                                                double theta,
                                                bool apply_ground_effect,
                                                double height) {
    std::vector<double> forces = getAeroForcesEarthframeVector(
        velocity, theta, apply_ground_effect, height);
    return {forces[0], forces[1]};
  }

  // Overload 2, if you dont want to explicitly tell it to not apply ground
  // effect
  std::array<double, 2>
  Aerodynamics::getAeroForcesEarthframe(std::array<double, 2> velocity,
                                        double theta) {
    std::vector<double> forces =
        getAeroForcesEarthframeVector(velocity, theta, false, 0e0);
    return {forces[0], forces[1]};
  }

  // Tempalted function. Can be called both with T=double and
  // T=CppAD::AD<double>
  template <typename T1, typename T2, typename T3>
  auto getAeroForcesEarthframeVector(std::array<T1, 2> velocity, T2 theta,
    ...
    if (apply_ground_effect) {
      auto cl_increase_factor = ground_effect_cl_increase_factor(
          height, _uav.getWingspan(), _uav.getAR());
      cl = cl_increase_factor * cl;
#ifdef DEBUG
      if (VERBOSITY_LEVEL >= 4)
        std::cout << "Ground effect (Aerodynamics::_Z): "
                  << "height = " << height
                  << "cl_increase_factor = " << cl_increase_factor << std::endl;
#endif
    }
    auto cd = _uav.getCd0() + pow(cl, 2) / (M_PI * _uav.getAR() * _uav.get_e());
    ///////////////////////////////////////
    ...
    std::array<decltype(T1() + T2()), 2> Fearth =
        utils::rotateFromWind2Earthframe(Fwind, getAoa1(velocity));
...
private:
  template <typename T> T _qinf(T velocity) {
