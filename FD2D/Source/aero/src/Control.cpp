...
:_engine(engine),_uav(uav), _aero(Aerodynamics()), _max_amps(max_amps), _current_thrust_mode(MAX){};
...
double Control::_getTheta(std::array<double, 2> velocity, double t, bool apply_ground_effect, double height){
    ...
    switch (_current_control_mode){
        case TAKEOFF:
            ...
            break;
        case TRIM_GAMMA:
            theta = _aero.getThetaForTrim(_uav, 
                                            velocity,
                                            *(this), apply_ground_effect, height);
...
std::array<double, 2> Control::getForces(const State &state, ControlState& control_state, double t){
    ...
    std::array<double, 2> u = _aero.getAeroForcesEarthframe(_uav, velocity, control_state.theta, apply_ground_effect, state.z + _uav.getWheelOffset());
    ...
    void Control::applyBoundaries(std::array<double, 2> &forces, double altitude){
        ...
        if ( (forces[1] < 1.3*_uav.getTotalMass()*9.81) && (altitude < 0.01) && (_current_control_mode == TAKEOFF)){
