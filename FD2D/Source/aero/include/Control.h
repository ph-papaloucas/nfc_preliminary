...
class Control {
public:
  enum ThrustMode { THRUST_UNDEFINED, MAX };
  ...
  enum TerminationMode { REACH_CEIL, REACH_DISTANCE };

  Control(const UAV &uav, const EngineMap &engine, double max_amps);

  void setControlMode(ControlMode control_mode,
                      std::array<double, 4>
                          coeffs); // set control for stuff that need polynomyal
  void setControlMode(ControlMode control_mode); // set control for stuff that
                                                 // dont need polynomial

  // void setControlMode(ControlMode control_mode){_current_control_mode =
  // control_mode;};
  void setTerminationMode(TerminationMode term_mode, double value) {
    _termination_mode = term_mode;
    _termination_value = value;
  }

  std::array<double, 2> getForces(const State &state,
                                  ControlState &control_state, double t);
  ...
  template <typename T> T getThrust(std::array<T, 2> velocity_bodyframe) const {
...
private:
  ...
  double _getTheta(std::array<double, 2> velocity, double t,
                   bool apply_ground_effect, double height);

  void _applyControl(ControlState &control_state,
